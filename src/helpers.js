...
//import './packery-mode.pkgd';
//import './slick';
//import bindDragScroll from './dragScroll';
//import Swiper from 'swiper';
//import SmoothScroll from 'smooth-scroll';

const screenHeight = (elem, multiplier) => {
...
const docReady_winResize_functions = () => {
...
const docReady_winResize_functions_mobile = () => {
...
const mobile_reserve = () => {
  if ($(window).width() < 768) {
    var $banner_height = $('.banner').height() + 65;
...
const componentDidMountJqueryStuff = () => {
  hotelRunnerSetup();
  try {
  ...
  function twinSliderCaption(direction) {
    if( direction == 'prev' ) {
      var $prevTwinSliderCaption = $twinSliderCaption.find('.twin-slider__caption--active').prev();
      ...
      var $nextTwinSliderCaption = $twinSliderCaption.find('.twin-slider__caption--active').next();
  ...
  var $category_text = $('.banner__category-container .banner__category-item').first().text();
  var $category_slug = $('.banner__category-container .banner__category-item').first().attr('data-category');
  var $person_text = $('.banner__person-container .banner__category-item').first().text();
  var $person_slug = $('.banner__person-container .banner__category-item').first().attr('data-category');
  ...
  $('.banner__category-select').on('click', function() {
    var $this_select = $(this).parent().find('.banner__category-group').hasClass('active');
  ...
  $('.banner__category-item').on('click', function() {
    ...
    $('.banner__category-group').removeClass('active');
    var $neighborhood_category = $('.neighborhood_category').val();
    var $category_slug = $('.banner__category-container .banner__category-label').attr('data-category');
    var $person_slug = $('.banner__person-container .banner__category-label').attr('data-category');
  });
  ...
  $.each($('.reserve-popup__filter-select .reserve-popup__select'), function( key, value ) {
    var selected = $(value).find('.selected');
...
const componentDidMountJqueryStuff2 = () => {
...
const hotelRunnerSetup = () => {
  // Hotel runner
  hr.asset_host = window.location.protocol + '//app.hotelrunner.com';
...
const initialize_hr_widget = () => {
  ...
  if (typeof(window.jQuery) == "undefined" || ((window.HotelRunnerLib.compare_versions('1.7.2', window.jQuery.fn.jquery) == '1') && (typeof(hjq) == 'undefined'))) {
    var old_jquery;
    var no_jquery = (typeof(window.jQuery) == "undefined")
    if (!no_jquery) {
      old_jquery = window.jQuery;
    }
    window.HotelRunnerLib.loadScript(hr['asset_host'] + "/assets/jquery.js", function () {
      hjq = window.jQuery.noConflict();
      if (!no_jquery) {
    ...
    var open_with_popup = function (url) {
        ...
        show: function (custom_url) {
          if (isMobile.iOS()) {
            ...
            if (!hr_prepared || (custom_url != url_was)) {
              window.HotelRunnerWidget.prepare(custom_url);
            }
            toggle_hr_viewport(true);
            window.HotelRunnerWidget.set_dimensions();
...
//                z.style.left = Math.max(0, y) + "px"
    ...
    if (typeof(window.booknow_button_id) == "undefined") {
      id = "booknow-hr-" + hr.align + hr.valign;
    ...
    $('#' + id).hover(function () {
      window.HotelRunnerWidget.prepare(null);
    });

    var handle_click = function (e) {
      e.preventDefault();
      window.HotelRunnerWidget.show(null);
    };
    ...
    var toggle_hr_viewport = function (enable) {
      if (enable) {
        ...
        var old_meta = $('meta[name=viewport]').clone();
