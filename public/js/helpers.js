...
const addBtnReseve = () => {
  [...document.getElementsByClassName("hotelRunnerReserveBtn")].forEach(
    (element, index, array) => {
      element.addEventListener("click", function(){ HotelRunnerWidget.show(null); });
    }
  );
};
...
const initialize_hr_widget = function () {
        ...
        if (typeof(window.booknow_button_id) == "undefined") {
          ...
          if (hr.widget_type == 'booknow') {
            if (hr.widget_version == 2) {
              inner = $("<div id='cm_preview_button_free_text' style=\"display: none; background-color:" + hr.background_color + ";color:" + hr.text_color + "\" class=\"" + hr_class + "\">" +
                "<p>" + hr.text + "</p>");
            } else {
              var hr_image;
...
window.componentDidMountJqueryStuff = componentDidMountJqueryStuff;
window.componentDidMountJqueryStuff2 = componentDidMountJqueryStuff2;
window.addBtnReseve = addBtnReseve;